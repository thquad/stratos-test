...
var listCNSIsByUser = `SELECT c.guid, c.name, c.cnsi_type, c.api_endpoint, c.doppler_logging_endpoint, t.user_guid, t.token_expiry, c.skip_ssl_validation, t.disconnected, t.meta_data, c.sub_type, c.meta_data as endpoint_metadata, created_by
...
func (p *PostgresCNSIRepository) ListByUser(userGUID string) ([]*interfaces.ConnectedEndpoint, error) {
	...
	q1rows, q1err := p.db.Query("SELECT guid, name, created_by FROM cnsis")
	if q1err != nil {
		log.Fatal(err)
	}
	for q1rows.Next() {
		var guid, name, createdBy string
		if err := q1rows.Scan(&guid, &name, &createdBy); err != nil {
			log.Fatal(err)
		}
		fmt.Println(guid + " / " + name + " / " + createdBy)
	}
	q1rows.Close()
